...
def find_latest_run_dir(rl_root: Path) -> Path | None:
    """
    Return newest subfolder under rl_root (by mtime), even if it does not yet
    contain td3_episode_metrics.csv. The caller will check for artifacts.
    """
    if not rl_root.exists():
        return None
    cand = [p for p in rl_root.iterdir() if p.is_dir()]
    return max(cand, key=lambda p: p.stat().st_mtime) if cand else None

def find_latest_baseline_run(baselines_root: Path) -> Path | None:
    """Newest subfolder under baselines_root that contains baseline_cccv_summary.csv."""
...
def load_rl_metrics_path(rl_dir: Path | None, rl_run_dir: Path | None) -> Path:
    """
...
def main():
    ...
    ap.add_argument("--baseline-dir", type=Path, default=None,
                    help="(Legacy) explicit folder containing baseline_cccv_summary.csv")
    ap.add_argument("--baseline-root", type=Path, default=Path("data/processed/baselines"),
                    help="Root folder that contains baseline run subfolders")
    ap.add_argument("--baseline-run", type=str, default=None,
                    help="Baseline run folder name under --baseline-root (e.g., 'cccv_smoke'). If omitted, auto-pick latest run in baseline-root unless --baseline-dir is given.")
    ...
    if args.baseline_dir is not None:
        ...
        if args.baseline_run is not None:
            baseline_dir = args.baseline_root / args.baseline_run
        else:
            latest = find_latest_baseline_run(args.baseline_root)
            if latest is None:
                raise FileNotFoundError(
                    f"No baseline run found under {args.baseline_root}. "
                    f"Provide --baseline-dir or --baseline-run."
                )
            baseline_dir = latest

    base_path = baseline_dir / "baseline_cccv_summary.csv"
    ...
    print(f"Sources -> baseline: {base_path}   RL: {rl_metrics_path} (run: {run_dir_for_msg})")
